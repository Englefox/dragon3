...
#include "vector.h"
#include "matrix.h"
// NOTE: Going to try to use the built in OpenMP to speed up Matrix operations
#include <omp.h>
...
namespace math
{
    ...
    // often we only care about the relative length differences 
    ...
    // perpendicular is only defined as is for 2D vectors
    template<typename T>
    ...
    inline matrix<m, n, T> transpose(matrix<m, n, T>& mat)
    {
        ...
        // note that we take the rows and cols as nxm instead of mxn this time.
