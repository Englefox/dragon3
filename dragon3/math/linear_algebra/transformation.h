...
namespace math
{
    ...
    // specialization on affine matrices with matrix and scale
    ...
    // rotation
    // --------------------
    // getting a direct rotation matrix is only allowed on 3 dimensions
    template <typename T>
    ...
    // translation
    // translations are only defined for 4-dimensional matrices/vectors
    template <typename T>
    ...
    matrix<4, 4, T> lookAt(vector<3, T>& position, vector<3, T>& target, vector<3, T>& worldUp)
    {
        ...
        frame[0].xyz = right;
        frame[1].xyz = up;
        //negative forward as we're looking towards negative z-axis
        frame[2].xyz = -forward; 
        // we want the inverse of the rotation part, which is equal
    ...
    matrix<4, 4, T> perspective(T fov, T aspect, T near_plane, T far_plane)
    {
        ...
        result[2][2] = -(far_plane + near_plane) / (far_plane - near_plane); //same as: (n + f) / (n - f)
        result[2][3] = -1.0f;

        result[3][2] = -(2.0f*near_plane*far_plane) / (far_plane - near_plane); //same as 2nf / (n-f)
        result[3][3] = 0.0f;
