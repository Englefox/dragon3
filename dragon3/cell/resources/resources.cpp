...
namespace Cell
{
	...
	SceneNode*  Resources::LoadAnimatedMesh(Renderer* renderer, std::string name, std::string path, AnimatedMeshLoader* amesh)
	{
		unsigned int id = SID(name);
		...
		// MeshLoader::LoadMesh initializes a scene node hierarchy on the heap. We are responsible 
		// for managing the memory; keep a reference to the root node of the model scene.
		AnimatedMeshLoader* temp = new AnimatedMeshLoader();
		SceneNode* node = temp->LoadAnimatedMesh(renderer, path);
		amesh = temp;
		Resources::m_Meshes[id] = node;
