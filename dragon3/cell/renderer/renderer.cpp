...
namespace Cell
{
    ...
    void Renderer::Init(GLADloadproc loadProcFunc)
    {
        ...
        Cell::Texture *hdrMap = Cell::Resources::LoadHDR("sky env", "D:/engle/dragon3/dragon3/build/textures/backgrounds/alley.hdr");
    ...
    void Renderer::PushRender(Mesh* mesh, Material* material, AnimatedMeshLoader* amesh, math::mat4 transform, math::mat4 prevFrameTransform)
    {
        ...
        m_CommandBuffer->Push(mesh, material, amesh, transform, prevFrameTransform, math::vec3(-99999.0f), math::vec3(99999.0f), target);
    }
    ...
    void Renderer::PushRender(SceneNode* node)
    {
        ...
        while (!nodeStack.empty())
        {
            ...
            if (node->Mesh)
            {
                ...
                m_CommandBuffer->Push(node->Mesh, node->Material, node->AMesh, node->GetTransform(), node->GetPrevTransform(), boxMinWorld, boxMaxWorld, target);
            }
            for(unsigned int i = 0; i < node->GetChildCount(); ++i)
                nodeStack.push(node->GetChildByIndex(i));
        }
    ...
    void Renderer::PushPostProcessor(Material* postProcessor)
    {
        ...
        m_CommandBuffer->Push(nullptr, postProcessor, nullptr);
    }
        ...
        while (!sceneStack.empty())
        {
            ...
            if (node->Mesh)
            {
                ...
                if (samplerUniforms.find("TexAlbedo") != samplerUniforms.end())
                {
                    ...
                    commandBuffer.Push(node->Mesh, materials[materials.size() - 1], nullptr,node->GetTransform());
                }
    ...
    void Renderer::renderCustomCommand(RenderCommand* command, Camera* customCamera, bool updateGLSettings)
    {
		...
		AnimatedMeshLoader* amesh = command->AMesh;
		...
		renderAMesh(mesh, material->GetShader(), amesh);
    }
        ...
        unsigned int mipLevel)
    {
        ...
        // command buffer)
        CommandBuffer commandBuffer(this);
        commandBuffer.Push(scene->Mesh, scene->Material, nullptr, scene->GetTransform());
        ...
        while (!childStack.empty())
        {
            ...
            childStack.pop();
            commandBuffer.Push(child->Mesh, child->Material, nullptr, child->GetTransform());
	...
	void Renderer::renderAMesh(Mesh* mesh, Shader* shader, AnimatedMeshLoader* m_mesh)
	{
		if (m_mesh == nullptr)
			return;
		Shader* ManShader = m_MaterialLibrary->ManShader;
		ManShader->Use();
		...
		for (unsigned int i = 0; i < 100; i++) {
			...
			m_boneLocation[i] = ManShader->getUniformLocation(Name);
		}
		for (unsigned int i = 0; i < Transforms.size(); i++) {
			glUniformMatrix4fv(m_boneLocation[i], 1, GL_TRUE, (const GLfloat*) &Transforms[i]); 
		    //m_mesh.SetBoneTransform(i, Transforms[i]);
		}
		//ManShader->SetMatrix();
		glBindVertexArray(mesh->m_VAO);

		for (unsigned int i = 0; i < m_mesh->m_Entries.size(); i++) {
			const unsigned int MaterialIndex = m_mesh->m_Entries[i].MaterialIndex;

			assert(MaterialIndex < m_mesh->m_Textures.size());

			if (m_mesh->m_Textures[MaterialIndex]) {
				m_mesh->m_Textures[MaterialIndex]->Bind(GL_TEXTURE0);
			}

			glDrawElementsBaseVertex(GL_TRIANGLES,
				m_mesh->m_Entries[i].NumIndices,
				GL_UNSIGNED_INT,
				(void*)(sizeof(unsigned int) * m_mesh->m_Entries[i].BaseIndex),
				m_mesh->m_Entries[i].BaseVertex);
		}
